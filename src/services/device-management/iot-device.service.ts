...
export class IoTDeviceService {
  ...
  async enrichSigFoxDevice(iotDevice: IoTDevice): Promise<SigFoxDeviceWithBackendDataDto> {
    ...
    const sigfoxGroup = await this.sigfoxGroupService.findOneByGroupId(sigfoxDevice.groupId, application.belongsTo.id);
  ...
  async delete(device: IoTDevice): Promise<DeleteResult> {
    ...
    await this.entityManager.transaction(async transactionManager => {
      ...
      if (device.type === IoTDeviceType.LoRaWAN) {
        ...
        this.logger.debug(`Deleting LoRaWANDevice ${lorawanDevice.id} / ${lorawanDevice.deviceEUI} in Chirpstack ...`);
  ...
  private async mapAndCreateLoRaWANDevice(
    ...
    try {
      ...
      const success = await this.chirpstackDeviceService.createOrUpdateDevice(chirpstackDeviceDto, lorawanDeviceEuis);
      if (success) {
  ...
  private async fixMQTTInternalBrokerTopics(dbIotDevices: IoTDevice[]) {
    ...
    for (const iotDevice of newMQTTInternalBrokers) {
      ...
      mqttInternalBrokerDevice.mqtttopicname = (await this.mqttService.createTopic(mqttInternalBrokerDevice)).topicName;
  ...
  private async createNewMQTTClients(dbIotDevices: IoTDevice[]) {
    ...
    try {
      this.internalMqttClientListenerService.createMQTTClients(newMQTTExternalBrokers as MQTTExternalBrokerDevice[]);
