...
export class SigFoxApiDeviceTypeService {
    ...
    private readonly OS2IOT_BACKEND_SIGFOX_CALLBACK_PATH = "/api/v1/sigfox-callback/data/bidir?apiKey={deviceTypeId}";
    ...
    async getAllByGroupIds(sigfoxGroup: SigFoxGroup, groupIds?: string[]): Promise<SigFoxApiDeviceTypeResponse> {
    ...
    async getById(sigfoxGroup: SigFoxGroup, id: string): Promise<SigFoxApiDeviceTypeContent> {
        const url = `${this.URL_BASE}/${id}`;
        return await this.genericService.get(url, sigfoxGroup);
    }

    async create(sigfoxGroup: SigFoxGroup, dto: CreateSigFoxApiDeviceTypeRequestDto): Promise<SigFoxApiIdReferenceDto> {
        await this.setDefaults(dto, sigfoxGroup);
        return await this.genericService.post(this.URL_BASE, dto, sigfoxGroup);
    }

    async update(group: SigFoxGroup, id: string, dto: CreateSigFoxApiDeviceTypeRequestDto): Promise<void> {
        const URL = `${this.URL_BASE}/${id}`;
    ...
    async addOrUpdateCallback(group: SigFoxGroup, deviceTypeId: string): Promise<void> {
        ...
        const response = await this.genericService.get<SigFoxApiCallbacksResponseDto>(url, group);
        const callback = response.data.find(x => x.url.startsWith(this.OS2IOT_BACKEND_URL));
        ...
        if (callback) {
            ...
            callbackId = (await this.genericService.post<SigFoxApiIdReferenceDto>(url, dto, group)).id;
        }
    ...
    private async setActiveDownlinkCallback(group: SigFoxGroup, deviceTypeId: string, callbackId: string) {
        await this.genericService.put(`${this.URL_BASE}/${deviceTypeId}/callbacks/${callbackId}/downlink`, {}, group);
    }
    ...
    private async setDefaults(dto: CreateSigFoxApiDeviceTypeRequestDto, sigfoxGroup: SigFoxGroup) {
        const sigfoxApiGroup = await this.usersService.getByUserId(sigfoxGroup.username, sigfoxGroup);
